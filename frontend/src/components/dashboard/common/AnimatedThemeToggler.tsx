...
type TransitionVariant = "circle" | "square" | "diamond" | "star"
...
function getClipPaths(
  ...
  switch (variant) {
    case "square": {
      ...
      const end = `${cx - h}px ${cy - h}px, ${cx + h}px ${cy - h}px, ${cx + h}px ${cy + h}px, ${cx - h}px ${cy + h}px`
...
export function AnimatedThemeToggler({ duration = 400, variant = "circle", className, ...props }: Props) {
